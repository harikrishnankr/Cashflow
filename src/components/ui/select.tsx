...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(
  ({ label, id, disabled, compact, containerClassName, className, children, ...props }, ref) => {
    ...
    return (
      <div className={cn("flex flex-col gap-1.75", containerClassName)}>
        {label && (
          <CapsLabel
            htmlFor={inputId}
            className={disabled ? "text-(--ink-4)" : undefined}
          >
            {label}
          </CapsLabel>
        )}
        <div className="relative">
          <select
            ...
            disabled={disabled}
            className={cn(
              "w-full font-sans text-sm border rounded-(--r-sm) px-4 pr-10 outline-none appearance-none",
              compact ? "h-9" : "h-12",
